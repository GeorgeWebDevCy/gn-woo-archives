<?php
/*
  Plugin Name: GN Woo Archives
  Plugin URI: https://github.com/GeorgeWebDevCY/gn-woo-archives
  Description: Archive old WooCommerce orders automatically.
  Version: 1.0
  Author: George Nicolaou
  Author URI: https://georgenicolaou.me
  License: GPLv2
  Text Domain: gn-woo-archives
  Tags: WooCommerce, Archive, Orders, Performance
 */

if (!defined('ABSPATH')) exit;

global $WcAOO;
$WcAOO = new GN_Woo_Archives;


class GN_Woo_Archives
{
    private $legacy_statuses = array();
    private $archived_statuses = array();
    private $textdomain = 'gn-woo-archives';
    ...
    public function __construct()
    {
        ...
        add_filter('plugin_action_links_gn-woo-archives/gn-woo-archives.php', array(&$this, 'settings_link'));

        // Manage cron tasks
        $this->manage_schedules();
        add_action('gn_woo_archives_cron_tasks', array($this, 'cron_tasks'));
    }
            ...
            $settings = apply_filters(
                'woocommerce_settings_archives', array(
                    ...
                    array(
                        ...
                        'id'      => 'gn_woo_archives_older_than_days',
                        'type'    => 'text',
                        'default' => '0',
                        'desc_tip' => true,
                    ),
    ...
    {
        if (!wp_next_scheduled('gn_woo_archives_cron_tasks')) {
            wp_schedule_event(strtotime('+1 day'), 'daily', 'gn_woo_archives_cron_tasks');
        }
    }
    ...
    function cron_tasks()
    {
        $max_age = get_option('gn_woo_archives_older_than_days');
        if (!$max_age) {
            return;
        }
        global $wpdb;
